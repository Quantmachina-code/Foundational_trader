"""Notebook-friendly TabPFN runner using prepared parquet panel data.

Usage patterns:
1) Jupyter notebook:
    from run_tabpfn import run_tabpfn_from_parquet
    preds_df, metrics = run_tabpfn_from_parquet(target_col="target_1")
...
def run_tabpfn_from_parquet(
    ...
    """Load parquet panel, run TabPFN, save predictions, and return results."""
    if target_col not in {"target_1", "target_2"}:
        raise ValueError("target_col must be one of: 'target_1', 'target_2'.")

    if not HAS_TABPFN:
        raise RuntimeError(
            "TabPFN is not available. Install tabpfn and place tabpfn*.ckpt in repo root."
        )
